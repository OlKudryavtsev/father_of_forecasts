  ...
  if (name === 'quiz') {
    return (
      <svg {...common}>
        ...
        <path d="M9 8h6M9 12h2M13 12h2M9 16h6" />
      </svg>
    );
  }
...
function quizStatusTone(status) {
  return ['running', 'registration_open'].includes(status) ? 'active' : status === 'finished' ? 'done' : 'neutral';
}

function formatQuizDate(value) {
  if (!value) return 'без времени старта';
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return 'без времени старта';
...
const QUIZ_TYPE_META = {
  choice_4: { label: '4 варианта · Миллионер', round: 'millionaire', roundTitle: 'Кто хочет стать миллионером?' },
  choice_2: { label: '2 варианта', round: 'choice_2', roundTitle: 'Выбор из двух' },
  true_false: { label: 'Правда / ложь', round: 'true_false', roundTitle: 'Правда или ложь' },
  more_less: { label: 'Больше / меньше', round: 'more_less', roundTitle: 'Больше или меньше' },
  yes_no: { label: 'Да / нет', round: 'yes_no', roundTitle: 'Да или нет' },
  jeopardy: { label: 'Своя игра', round: 'jeopardy', roundTitle: 'Своя игра' },
  one_of_two: { label: 'Один из двух', round: 'one_of_two', roundTitle: 'Один из двух' },
  what_where_when: { label: 'Что? Где? Когда?', round: 'what_where_when', roundTitle: 'Что? Где? Когда?' },
  countdown: { label: 'Обратный отсчёт', round: 'countdown', roundTitle: 'Обратный отсчёт' },
...
// Ready quiz packs stay data-driven: they assemble only from the approved
// question bank of the active league and never create a session by themselves.
const QUIZ_PRESETS = [
  {
    id: 'quick_10',
    label: 'Квиз на 10 минут',
    description: '10 коротких вопросов без длинных форматов.',
    title: 'Квиз на 10 минут',
    count: 10,
    types: ['choice_4', 'choice_2', 'true_false', 'more_less', 'yes_no'],
    timerProfile: 'quick',
  },
  {
    id: 'hundred_to_one',
    label: 'Только Сто к одному',
    description: '5 рейтинговых вопросов 100к1.',
    title: 'Сто к одному',
    count: 5,
    types: ['hundred_to_one'],
  },
  {
    id: 'world_cup_history',
    label: 'История ЧМ',
    description: '10 вопросов из истории чемпионатов мира.',
  ...
  {
    id: 'world_cup_2026',
    label: 'Текущий ЧМ-2026',
    description: '10 вопросов о турнире 2026.',
    title: 'Чемпионат мира 2026',
    count: 10,
    matcher: 'current_2026',
  },
];
...
function defaultQuizOptions(type) {
  if (type === 'choice_4') return ['', '', '', ''];
  if (type === 'true_false') return ['Правда', 'Ложь'];
  if (type === 'more_less') return ['Больше', 'Меньше'];
  if (type === 'yes_no') return ['Да', 'Нет'];
  if (type === 'choice_2') return ['', ''];
  return [];
}

function defaultQuizPoints(type) {
  return { one_of_two: '300', what_where_when: '500', countdown: '500', hundred_to_one: '1000', jeopardy: '100' }[type] || '100';
}

function quizBankSummary(question) {
  const payload = question.question_payload || {};
  if (question.question_type === 'jeopardy') return `Тема: ${payload.topic || '—'} · неверный ответ: −${question.default_points}`;
  if (question.question_type === 'countdown') return '3 подсказки · один ответ на игрока';
  if (question.question_type === 'hundred_to_one') return 'Топ-10 · 100–1000 очков';
  if (['one_of_two', 'what_where_when'].includes(question.question_type)) return `Текстовый ответ · вариантов: ${(payload.answer_aliases || []).length}`;
  return '';
}
...
function QuizScreen({ activeLeagueId, leaguesData, initialQuizId = null }) {
  ...
  // A preset or copied finished quiz is a draft only in the Planner. It becomes
  // a live session only after the host presses «Создать квиз».
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      const result = await api(`/api/webapp/quizzes?league_id=${activeLeagueId}`);
  ...
  const loadDetail = useCallback(async (quizId = selectedQuizId, silent = false) => {
    ...
    try {
      const result = await api(`/api/webapp/quizzes/${quizId}`);
      setDetail(result);
      setError(null);
    } catch (err) {
      setError(err);
  ...
  function changeQuestionType(value) {
    ...
    setQuestionOptions(defaultQuizOptions(value));
    setCorrectIndex(0);
    setQuestionPoints(defaultQuizPoints(value));
    setQuestionAliases('');
    setQuestionTopic('');
    setCountdownFacts(['', '', '']);
  ...
  async function register() {
    ...
    setBusy(true);
    try {
      const result = await api(`/api/webapp/quizzes/${detail.quiz.id}/register`, { method: 'POST' });
      setDetail(result);
      await loadList();
      trackAnalytics('quiz_register', { screen: 'quiz', properties: { league_id: activeLeagueId || 0, quiz_id: detail.quiz.id } });
    } catch (err) {
      setError(err);
    } finally {
      setBusy(false);
  ...
  async function answer(optionKey) {
    ...
    const question = detail?.current_question;
    if (!quiz?.id || !question?.id || question.status !== 'open') return;
    setBusy(true);
    try {
      const result = await api(`/api/webapp/quizzes/${quiz.id}/questions/${question.id}/answer`, {
        method: 'POST',
        body: JSON.stringify({ selected_option_key: optionKey }),
      });
      setDetail(result);
      await loadList();
      trackAnalytics('quiz_answer', { screen: 'quiz', properties: { league_id: activeLeagueId || 0, quiz_id: quiz.id, question_id: question.id } });
    } catch (err) {
      setError(err);
    } finally {
      setBusy(false);
  ...
  async function submitTextAnswer(event) {
    ...
    const quiz = detail?.quiz;
    const question = detail?.current_question;
    if (!quiz?.id || !question?.id || !textAnswer.trim()) return;
    setBusy(true);
    try {
      const result = await api(`/api/webapp/quizzes/${quiz.id}/questions/${question.id}/text-answer`, {
        method: 'POST', body: JSON.stringify({ answer_text: textAnswer.trim() }),
      });
      setDetail(result);
      setTextAnswer('');
      await loadList();
  ...
  async function openJeopardyCell(questionId) {
    const quiz = detail?.quiz;
    if (!quiz?.id || !questionId) return;
    setBusy(true);
    try {
  ...
  function renderJeopardyBoard() {
    ...
    return (
      <section className="jeopardy-board-card" aria-label={`Табло Своя игра: выбрано ${chosen} из ${total} вопросов`}>
  ...
  async function adminAction(action) {
    ...
    setBusy(true);
    try {
      const result = await api(`/api/webapp/quizzes/${detail.quiz.id}/${action}`, { method: 'POST' });
      setDetail(result);
      await loadList();
      trackAnalytics('quiz_admin', { screen: 'quiz', properties: { league_id: activeLeagueId || 0, quiz_id: detail.quiz.id, quiz_status: action } });
    } catch (err) {
      setError(err);
    } finally {
      setBusy(false);
  ...
  async function approveQuestion(questionId) {
    setBusy(true);
    try {
      await api(`/api/webapp/quiz-bank/questions/${questionId}/approve?league_id=${activeLeagueId}`, { method: 'POST' });
      await loadBank();
    } catch (err) {
      setError(err);
    } finally {
  ...
  function questionPlannerSearchText(question) {
    return [question.question_text, question.tags, ...(question.topics || [])]
      .filter(Boolean).join(' ').toLocaleLowerCase('ru-RU');
  }

  function orderPlannerQuestions(rows) {
    return [...rows].sort((left, right) => (
      Number(left.times_used || 0) - Number(right.times_used || 0)
      || Number(left.id) - Number(right.id)
    ));
  }

  function pickRoundRobinQuestions(rows, types, count) {
    const buckets = new Map();
    const requestedTypes = types?.length ? types : [...new Set(rows.map((row) => row.question_type))];
    requestedTypes.forEach((type) => buckets.set(type, orderPlannerQuestions(rows.filter((row) => row.question_type === type))));
    const selected = [];
    let cursor = 0;
    while (selected.length < count) {
      const available = requestedTypes.filter((type) => (buckets.get(type) || []).length);
      if (!available.length) break;
      const type = available[cursor % available.length];
      const bucket = buckets.get(type) || [];
      selected.push(bucket.shift());
      cursor += 1;
    }
    if (selected.length < count) {
      const chosen = new Set(selected.map((row) => Number(row.id)));
      orderPlannerQuestions(rows).forEach((row) => {
        if (selected.length < count && !chosen.has(Number(row.id))) {
          selected.push(row); chosen.add(Number(row.id));
        }
      });
    }
  ...
  function buildCustomRounds(rows) {
    const groups = new Map();
    rows.forEach((question) => {
      const meta = QUIZ_TYPE_META[question.question_type] || { round: question.question_type, roundTitle: 'Раунд квиза' };
      if (!groups.has(meta.round)) groups.set(meta.round, { title: meta.roundTitle, round_type: meta.round, question_ids: [] });
      groups.get(meta.round).question_ids.push(Number(question.id));
    });
    return [...groups.values()].sort((left, right) => (QUIZ_ROUND_SEQUENCE[left.round_type] || 999) - (QUIZ_ROUND_SEQUENCE[right.round_type] || 999));
  }

  function presetCandidates(preset) {
    let rows = [...approvedBank];
    if (preset.types?.length) rows = rows.filter((question) => preset.types.includes(question.question_type));
    if (preset.matcher === 'history') {
      rows = rows.filter((question) => {
        const text = questionPlannerSearchText(question);
        return ['история чм', 'история чемпионатов мира', 'world cup history', 'чемпионатах мира', 'чемпионата мира'].some((needle) => text.includes(needle));
      });
    }
    if (preset.matcher === 'current_2026') {
      rows = rows.filter((question) => {
        const text = questionPlannerSearchText(question);
        return ['чм-2026', 'чм 2026', 'wc2026', 'world cup 2026', '2026'].some((needle) => text.includes(needle));
      });
    }
    return orderPlannerQuestions(rows);
  }

  function applyQuizPreset(preset) {
    const candidates = presetCandidates(preset);
    const chosen = pickRoundRobinQuestions(candidates, preset.types, preset.count);
    if (chosen.length < preset.count) {
      setError(new Error(`Для набора «${preset.label}» нужно ${preset.count} одобренных вопросов, а подходит только ${chosen.length}. Дополните Банк или выберите другой набор.`));
      return;
    }
    setPlannerMode('preset');
    setCustomPlan({ id: preset.id, label: preset.label, description: preset.description, rounds: buildCustomRounds(chosen) });
    setSelectedQuestionIds(chosen.map((question) => Number(question.id)));
    setQuizTitle(preset.title);
    setScheduledStart('');
    if (preset.timerProfile === 'quick') {
      setTimerSettings((current) => ({
        ...current,
        choice_4: 20, choice_2: 20, true_false: 20, more_less: 20, yes_no: 20,
      }));
      setRevealSeconds('7');
    }
    setSeedMessage(`Собран набор «${preset.label}»: ${chosen.length} вопросов. При необходимости измените название и время старта перед созданием.`);
  }

  async function copyQuizToPlanner() {
    if (!detail?.quiz || !detail?.rounds?.length) return;
    setBusy(true);
    try {
      const result = await api(`/api/webapp/quiz-bank/questions?league_id=${activeLeagueId}`);
      const approvedRows = (result.questions || []).filter((question) => question.status === 'approved');
      setBank(result.questions || []);
      const allowed = new Set(approvedRows.map((question) => Number(question.id)));
      const copiedRounds = detail.rounds.map((round) => ({
        title: round.title || 'Раунд квиза',
        round_type: round.round_type,
        question_ids: (round.bank_question_ids || []).map(Number).filter((id) => allowed.has(id)),
      })).filter((round) => round.question_ids.length);
      const originalCount = detail.rounds.reduce((sum, round) => sum + (round.bank_question_ids || []).length, 0);
      const copiedIds = copiedRounds.flatMap((round) => round.question_ids);
      if (!copiedIds.length || copiedIds.length !== originalCount) {
        throw new Error('Не все вопросы исходного квиза доступны и одобрены в Банке. Восстановите их из архива или создайте копию вручную.');
      }
      setPlannerMode('preset');
      setCustomPlan({ id: 'copy', label: `Копия: ${detail.quiz.title}`, description: 'Черновик из завершённого квиза. Участники и результаты не копируются.', rounds: copiedRounds });
      setSelectedQuestionIds(copiedIds);
      setQuizTitle(`Копия: ${detail.quiz.title}`.slice(0, 160));
      setScheduledStart(''); setIsTestRun(false); setTestChatId('');
      setWorkspaceTab('planner'); setBankOpen(true);
      setSeedMessage(`Черновик собран: ${copiedIds.length} вопросов из «${detail.quiz.title}». Новый квиз ещё не создан и уведомления не отправлялись.`);
    } catch (err) {
      setError(err);
    } finally {
      setBusy(false);
  ...
  async function createQuiz(event) {
    ...
    if (plannerMode === 'preset') {
      if (!customPlan?.rounds?.length) {
        setError(new Error('Сначала выберите готовый набор или скопируйте завершённый квиз.'));
        return;
      }
      const plannedIds = customPlan.rounds.flatMap((round) => round.question_ids.map(Number));
      const selectedIds = selectedBank.map((question) => Number(question.id));
      if (plannedIds.length !== selectedIds.length || plannedIds.some((id) => !selectedIds.includes(id))) {
        setError(new Error('Состав набора изменился. Соберите набор заново или выберите вопросы вручную.'));
        return;
      }
    }
    ...
    if (plannerMode !== 'random' && plannerMode !== 'preset') {
    ...
    try {
      ...
      const grouped = plannerMode === 'preset'
        ? customPlan.rounds.map((round) => ({ ...round, question_ids: [...round.question_ids] }))
        : buildCustomRounds(selectedBank).map((round) => ({
          ...round,
          title: plannerMode === 'random' ? 'Случайный вопрос' : round.title,
        }));
  ...
  const visibleBank = useMemo(() => {
    const query = bankQuery.trim().toLocaleLowerCase('ru-RU');
    return bank.filter((question) => {
      const matchesType = bankTypeFilter === 'all' || question.question_type === bankTypeFilter;
      if (!matchesType) return false;
      if (!query) return true;
      const haystack = [question.question_text, question.type_label, question.tags, ...(question.topics || [])]
        .filter(Boolean).join(' ').toLocaleLowerCase('ru-RU');
  ...
  return (
    <main className="screen-content quiz-screen">
      <div className="section-label quiz-section-label">Квиз · {activeLeague?.name || 'Лига'}</div>
      ...
      <nav className="quiz-workspace-tabs" aria-label="Разделы квиза">
        {QUIZ_WORKSPACE_TABS.filter((tab) => tab.id === 'games' || (tab.id === 'planner' && canHost) || (tab.id === 'bank' && canEdit)).map((tab) => (
          <button key={tab.id} type="button" className={workspaceTab === tab.id ? 'active' : ''} aria-current={workspaceTab === tab.id ? 'page' : undefined} onClick={() => {
        ...
        <>
          ...
          {detailLoading && !detail ? <LoadingCard text="Открываю квиз..." /> : quiz && (
            <section className="card quiz-live-card">
              <header className="quiz-live-head">
                <div><span className={`quiz-status ${quizStatusTone(quiz.status)}`}>{quizStatusLabel(quiz.status)}</span><h2>{quiz.title}</h2></div>
                <b>{quiz.my_score || 0}<small> очк.</small></b>
              </header>
              <p className="muted quiz-meta">{quiz.questions_total} вопросов · зарегистрировано: {quiz.registered_count}{quiz.scheduled_start_at ? ` · старт: ${formatQuizDate(quiz.scheduled_start_at)}` : ''}</p>
              {quiz.can_manage && ['finished', 'cancelled'].includes(quiz.status) && <button type="button" className="quiz-copy-draft-button" disabled={busy} onClick={copyQuizToPlanner}>Копировать в План</button>}

              {quiz.status === 'registration_open' && !quiz.is_registered && <button className="quiz-primary-button" disabled={busy} onClick={register}>Участвовать в квизе</button>}
              {quiz.status === 'registration_open' && quiz.is_registered && <div className="quiz-registered">✓ Вы зарегистрированы. Ждём старта.</div>}
              {quiz.status === 'paused' && <div className="quiz-paused">⏸ Игра на паузе. Таймер остановлен ведущим.</div>}

              {currentQuestion && (
                <article className={`quiz-question-card ${currentQuestion.status} ${currentQuestion.type}`}>
                  <div className="quiz-question-top">
                    <span>{currentQuestion.round_title || 'Раунд'} · вопрос {currentQuestion.order}</span>
                    <b>{currentQuestion.type === 'countdown' ? '500 / 300 / 100' : `${currentQuestion.points} очк.`}</b>
                  </div>
                  ...
                  {currentQuestion.answer_mode === 'choice' && <div className="quiz-options">
                    {currentQuestion.options.map((option) => {
                      const selected = currentQuestion.answer?.selected_option_key === option.key;
                      const revealedWrong = currentQuestion.status === 'revealed' && selected && !option.is_correct;
                      return <button key={option.key} type="button" disabled={busy || !quiz.is_registered || currentQuestion.status !== 'open'} onClick={() => answer(option.key)} className={`${selected ? 'selected' : ''} ${option.is_correct ? 'correct' : ''} ${revealedWrong ? 'wrong' : ''}`}>
                        <b>{option.key}</b><span>{option.text}</span>
                      </button>;
                    })}
                  </div>}

                  {currentQuestion.answer_mode === 'text' && currentQuestion.status === 'open' && (
                    currentQuestion.answer?.is_locked ? <p className="quiz-answer-saved">🔒 Ответ зафиксирован на подсказке {currentQuestion.answer?.answer_stage}. Изменить его нельзя.</p> : (
                      <form className="quiz-text-answer" onSubmit={submitTextAnswer}>
                        <input value={textAnswer} onChange={(event) => setTextAnswer(event.target.value)} placeholder={currentQuestion.type === 'one_of_two' ? 'Назовите хотя бы одного игрока' : 'Введите ответ'} disabled={busy || !quiz.is_registered} />
                        <button type="submit" disabled={busy || !quiz.is_registered || !textAnswer.trim()}>Ответить</button>
                      </form>
                    )
                  )}
                  {currentQuestion.status === 'open' && currentQuestion.answer?.selected_option_key && <p className="quiz-answer-saved">Ответ принят. До конца времени можно выбрать другой вариант.</p>}
                  {currentQuestion.status === 'open' && currentQuestion.answer?.answer_text && !currentQuestion.answer?.is_locked && <p className="quiz-answer-saved">Ответ принят: «{currentQuestion.answer.answer_text}». Его можно изменить до закрытия вопроса.</p>}
                  {currentQuestion.status === 'revealed' && <div className="quiz-reveal">
                    <b>{currentQuestion.answer?.is_correct ? `Верно · ${Number(currentQuestion.answer.points_awarded || 0) >= 0 ? '+' : ''}${currentQuestion.answer.points_awarded || 0}` : currentQuestion.answer ? `Неверно · ${currentQuestion.answer.points_awarded || 0}` : 'Ответ не получен'}</b>
                    {currentQuestion.correct_answer && <p><strong>Правильный ответ:</strong><br />{currentQuestion.correct_answer}</p>}
              ...
              <section className="quiz-scoreboard">
                ...
                {(detail.scoreboard || []).length ? <ol>{detail.scoreboard.map((row) => <li key={row.user_id} className={row.user_id === (detail?.quiz?.my_user_id) ? 'me' : ''}><b>{row.place}</b><span>{row.display_name}</span><strong>{row.score_total}</strong></li>)}</ol> : <p className="muted">Участники появятся после регистрации.</p>}
              </section>

              {quiz.can_manage && (
                <div className="quiz-admin-actions">
                  {quiz.status === 'registration_open' && <button type="button" disabled={busy} onClick={() => adminAction('start')}>▶ Запустить сейчас</button>}
                  {quiz.status === 'running' && <><button type="button" disabled={busy} onClick={() => adminAction('pause')}>⏸ Пауза</button><button type="button" disabled={busy} onClick={() => adminAction('close-current')}>Закрыть вопрос</button></>}
          ...
          {workspaceTab !== 'games' && canManage && (
            <section className="card quiz-admin-card">
              <div className="quiz-admin-toggle quiz-admin-heading">
                <span><b>{workspaceTab === 'bank' ? 'Банк вопросов' : 'Планирование квиза'}</b><small>{workspaceTab === 'bank' ? 'создание, импорт и загрузка наборов' : 'сценарий, таймеры и набор вопросов'}</small></span>
              </div>
              {bankOpen && <div className="quiz-admin-body">
                {workspaceTab === 'bank' && canEdit && <>
                <details className="quiz-bank-fold" open={bankPanels.seed} onToggle={(event) => { const open = event.currentTarget?.open === true; setBankPanels((current) => ({ ...current, seed: open })); }}>
                ...
                <details className="quiz-bank-fold" open={bankPanels.editor} onToggle={(event) => { const open = event.currentTarget?.open === true; setBankPanels((current) => ({ ...current, editor: open })); }}>
                ...
                <form id="quiz-question-editor" className="quiz-form" onSubmit={createQuestion}>
                  ...
                  <label>Вопрос<textarea value={questionText} onChange={(event) => setQuestionText(event.target.value)} placeholder={questionType === 'countdown' ? 'Кого или что нужно угадать по подсказкам?' : 'Текст вопроса'} required /></label>

                  {isQuizChoiceType(questionType) && <div className="quiz-option-editor">{questionOptions.map((value, index) => <label key={index}><span><input type="radio" name="quiz-correct" checked={Number(correctIndex) === index} onChange={() => setCorrectIndex(index)} /> {String.fromCharCode(65 + index)} · правильный</span><input value={value} onChange={(event) => setQuestionOptions((current) => current.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} placeholder={`Вариант ${String.fromCharCode(65 + index)}`} required /></label>)}</div>}
                ...
                <section className="quiz-bank-list">
                  ...
                  <div className="quiz-bank-toolbar">
                    <input value={bankQuery} onChange={(event) => setBankQuery(event.target.value)} placeholder="Поиск по тексту, теме или тегу" />
                    <select value={bankTypeFilter} onChange={(event) => setBankTypeFilter(event.target.value)}>
                      <option value="all">Все форматы</option>
                      {Object.entries(QUIZ_TYPE_META).map(([key, meta]) => <option key={key} value={key}>{meta.label}</option>)}
                    </select>
                  </div>
                  <details className="quiz-bank-fold quiz-import-fold" open={bankPanels.import} onToggle={(event) => { const open = event.currentTarget?.open === true; setBankPanels((current) => ({ ...current, import: open })); }}><summary>Импортировать вопросы из JSON</summary><div className="quiz-import-box"><textarea value={importText} onChange={(event) => setImportText(event.target.value)} placeholder='{ "questions": [ ... ] }' /><button type="button" disabled={busy || !importText.trim()} onClick={importBank}>Импортировать как черновики</button></div></details>
                ...
                {workspaceTab === 'planner' && canHost && <form className="quiz-form quiz-create-form" onSubmit={createQuiz}>
                  ...
                  <div className="quiz-planner-modes" aria-label="Режим квиза">
                    <button type="button" className={plannerMode === 'full' ? 'active' : ''} onClick={() => { setPlannerMode('full'); setPlannerRound('millionaire'); setSelectedQuestionIds([]); setCustomPlan(null); }}>Полный</button>
                    <button type="button" className={plannerMode === 'single' ? 'active' : ''} onClick={() => { setPlannerMode('single'); setPlannerRound('millionaire'); setSelectedQuestionIds([]); setCustomPlan(null); }}>Формат</button>
                    <button type="button" className={plannerMode === 'random' ? 'active' : ''} onClick={() => { setPlannerMode('random'); setPlannerRound('any'); setSelectedQuestionIds([]); setCustomPlan(null); }}>Случайный</button>
                    <button type="button" className={plannerMode === 'preset' ? 'active' : ''} onClick={() => { setPlannerMode('preset'); setSelectedQuestionIds([]); setCustomPlan(null); }}>Наборы</button>
                  </div>
                  <p className="quiz-planner-mode-hint">{plannerMode === 'full' ? '60 вопросов · 9 раундов' : plannerMode === 'single' ? 'Один формат · 5 вопросов, «Своя игра» — 20' : plannerMode === 'random' ? 'Один вопрос из выбранного формата' : 'Быстрые готовые сценарии и копия завершённого квиза как черновик.'}</p>
                  ...
                  {plannerMode === 'preset' && <section className="quiz-preset-picker">
                    <div className="quiz-preset-picker-head"><b>Готовые наборы</b><small>Собираются только из одобренных вопросов текущей лиги.</small></div>
                    <div className="quiz-preset-grid">{QUIZ_PRESETS.map((preset) => <button key={preset.id} type="button" className={customPlan?.id === preset.id ? 'active' : ''} disabled={busy || !approvedBank.length} onClick={() => applyQuizPreset(preset)}><b>{preset.label}</b><small>{preset.description}</small></button>)}</div>
                    {customPlan && <div className="quiz-preset-summary"><b>{customPlan.label}</b><span>{customPlan.description}</span><small>{customPlan.rounds.length} раунд(а) · {customPlan.rounds.reduce((sum, row) => sum + row.question_ids.length, 0)} вопросов</small></div>}
                  </section>}
                  <label>Название<input value={quizTitle} onChange={(event) => setQuizTitle(event.target.value)} placeholder={plannerMode === 'random' ? 'Случайный вопрос' : plannerMode === 'preset' ? 'Название готового набора' : 'Например, Футбольный квиз №1'} required /></label>
                  ...
                  {plannerMode === 'preset' ? <div className="quiz-question-picker quiz-preset-questions">
                    {customPlan ? <><div className="planner-picker-head"><div><b>Состав черновика</b><p>{selectedQuestionIds.length} вопросов · изменение состава через выбранный набор или повторное копирование.</p></div></div>{customPlan.rounds.map((round) => <div className="quiz-preset-round" key={`${round.round_type}-${round.title}`}><b>{round.title}</b><small>{round.question_ids.length} вопросов</small></div>)}</> : <div className="planner-empty-bank"><p className="muted">Выберите готовый набор выше или откройте завершённый квиз и нажмите «Копировать в План».</p></div>}
                  </div> : <div className="quiz-question-picker">
                    ...
                    }} /><span><b>{question.type_label}</b> · {question.default_points} · {question.question_text}</span></label>) : <div className="planner-empty-bank"><p className="muted">В Банке пока нет одобренных вопросов для этого раунда.</p><button type="button" onClick={() => { setWorkspaceTab('bank'); setBankOpen(true); }}>Открыть Банк вопросов</button></div>}
                  </div>}
                  ...
                  <button type="submit" className="quiz-primary-button" disabled={busy || !selectedQuestionIds.length || (plannerMode === 'preset' && !customPlan)}>{isTestRun ? `Создать тестовый прогон (${selectedQuestionIds.length})` : plannerMode === 'random' ? 'Запустить случайный вопрос' : plannerMode === 'preset' ? `Создать из набора (${selectedQuestionIds.length})` : `Создать квиз (${selectedQuestionIds.length})`}</button>
                </form>}
