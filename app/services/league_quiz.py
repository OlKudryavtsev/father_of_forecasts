...
def _quiz_event_dedupe_key(event_type: str, payload: dict | None) -> tuple | None:
    ...
    if event_type in {"quiz_created", "quiz_started", "quiz_finished"}:
        return (event_type,)
    if event_type in {"round_started", "round_finished"}:
        return (event_type, int(data.get("round_id") or 0) or int(data.get("round_order") or 0))
    ...
    if key:
        ...
        existing = (
            ...
            .filter(
                ...
                LeagueQuizEvent.event_type == event_type,
            )
            .order_by(LeagueQuizEvent.id.desc())
            .all()
        )
